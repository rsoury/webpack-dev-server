...
function Server(compiler, options, _log) {
  ...
  const features = {
    ...
    /* an option to include session and cookie store per webpack-dev-server instance. */
    stateful: () => {
      app.use(session({
        secret: "some-webpack-dev-server-secret"
      }))
    }
  };
  ...
  if(options.stateful){ defaultFeatures.push('stateful') }
